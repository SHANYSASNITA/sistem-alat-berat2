    ...
    function addActiveClass(element) {
        ...
        if (current === "") {
            ...
            if (element.getAttribute("href").indexOf("index.html") !== -1) {
                // Checking href of 'element' matching with 'index.html'
                ...
                if (getParents(element, ".sub-menu").length) {
                    // Checking if it's a submenu 'element'
            ...
            if (element.getAttribute("href").indexOf(current) !== -1) {
                // Checking href of 'element' matching with current url
                const elParents = getParents(element, ".nav-item"); // Getting parents of the 'element' with a class '.nav-item'
                elParents[elParents.length - 1].classList.add("active"); // Adding class 'active' to the outer(direct) '.nav-item'
                if (getParents(element, ".sub-menu").length) {
                    // Checking if it's a submenu 'element' [in vertical menu sidebar - demo1]
                    element.closest(".collapse").classList.add("show"); // Adding class 'show' to the closest '.collapse' to expand submenu
                    element.classList.add("active"); // Adding class 'active' to the submenu '.nav-link'
                }
                if (getParents(element, ".submenu-item")) {
                    // Checking if it's a submenu-item 'element' [in horizontal menu bottom-navbar - demo2]
                    element.classList.add("active"); // Adding class 'active' to the submenu-item '.nav-link'
                    if (element.closest(".nav-item.active .submenu")) {
                        // Checking element has a submenu
                        element
                            .closest(".nav-item.active")
                            .classList.add("show-submenu"); // adding class 'show-submenu' to the parent .nav-item (only for mobile/tablet)
                    }
                }
            }
        }
